...
def personalize_atlas_for_subject(
    ...
    # affine[:3, :3] *= 2.0  # increase voxel size (downsample in space)

    # mask_img_ds = resample_img(
    #   mask_img,
    #   target_affine=affine,
    #   interpolation='nearest'  # important to preserve mask labels
    # )
...
brainnetome_atlas_file = "../Atlases/tpl-MNI152NLin2009cAsym_atlas-brainnetomeCombined_dseg.nii.gz"
    ...
    if (
        ...
        for subjects in os.listdir(path_data + "/" + site):
            ...
            if site == "Beijing":
                ...
                subject_val = subjects.replace("D", "d").replace("P", "p")
            elif site == "UMN":
                #   subject_mask = subjects.replace("_","")
                subject_val = subjects.replace("M", "m")
            elif site == "Ghent":
                #   subject_mask = subjects.replace("_","")
            ...
            if site == "Lawson":
                if os.listdir(path_data + "/" + site).index(subjects) <= 22:
                    continue
            ...
            if "." not in subjects and not os.path.exists(
                ...
                + "/"
                + subjects
                + f"/{subject_val}_schaefer_mni_image.nii.gz"
                ...
                if os.path.exists(mask_file):
                    ...
                    subj_atlas_brainnetome, mask_subj_img_brainnetome = (
                        personalize_atlas_for_subject(
                            ...
                            + "/"
                            + subjects
                            + f"/{subject_val}_brainnetome_mni_image.nii.gz",
                            min_voxels=20,
                            dilate=False,
                            dilation_iters=1,
                        )
