...
if __name__ == "__main__":
    ...
    out_path = f"./visualization_test_projections/folder_4_struct_only_{iterations}_{batch_size}_{learning_rate}_{loss_selector}_{out_dim}_{rs_data_model_sel}_{temperature}"
    ...
    iter, _ = load_latest_ckpt_struct(model_path, device,
                               enc_surf, enc_thick, enc_vol, optimizer_SSL, scheduler_SSL, iteration_ckpth)

    for model in [enc_thick, enc_surf, enc_vol]:
        model.to(device)
    ...
    # multiply by two given the two modalities in this experiment, vary it if
    # this necessary..
    sites_evals_long = np.repeat(sites_evals, 3)
    ...
    age_binary_long = np.repeat(age_binary, 3)
