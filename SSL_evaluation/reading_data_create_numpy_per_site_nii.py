...
def save_rs_site_h5(
    ...
    """
    Saves large 4D tensors to HDF5 efficiently.
    rs_tensor_4d_* expected shape: (N, X, Y, Z, T)
    """
...
def load_4d_tensor(file_map, dtype=np.float32, memmap_path=None, atlas=None):
    ...
    for i, key in enumerate(keys_sorted[0:], start=0):
        path = file_map[key]
        # img = nib.load(path)
        ...
        #    new_shape = (np.array([193, 229, 193], dtype=int)).astype(int)  # [X*2, Y*2, Z*2]
        ...
        #    logger.info(f"upsampling image for subject {key} shape of image is {orig_shape}")
            ...
            if "." not in site:
                ...
                for subjects in os.listdir(path_dir + "/" + site):
                    ...
                    pattern = os.path.join(
                        "/".join([path_dir, site, subjects]), "*.nii"
                    )
                ...
                if type_data == "rs_data":
                    ...
                    rs_tensor_3d_dkt_voll, _, _, _ = load_3d_tensor(
                        vol_3d_map_dkt,
                        dtype=np.float32,
                ...
                if type_data == "rs_data":
                    ...
                    np.savez_compressed(
                        ...
                        rs_tensor_3d_dkt_voll=rs_tensor_3d_dkt_vol,
