...
def get_concat_embedding(rs_DATA, st_DATA,
    ...
    if len(val_clust) > 1:
        ...
        nmi_mod = compute_nmi_via_kmeans(
            X_all, val_clust, n_clusters=batch_sizes)
    else:
        sil_mod = np.nan
        nmi_mod = np.nan
...
if __name__ == "__main__":
    ...
    out_path = f"./visualization_test_projections/folder_4_cort_{iterations}_{batch_size}_{learning_rate}_{loss_selector}_{out_dim}_{rs_data_model_sel}_{temperature}"

    subject_indices_current_data = "/home/mayortorresjm/ENIGMA-PTSD/Data/npz/subjects_overlaped_all_modalities.npz"

    # load the blank models first to continue with the model loading
    ...
    iter, _ = load_latest_ckpt_cort(model_path, device, enc_4D_rsdata,
                               enc_thick, optimizer_SSL, scheduler_SSL, iteration_ckpth)
    ...
    # multiply by two given the two modalities in this experiment, vary it if
    # this necessary..
    sites_evals_long = np.repeat(sites_evals, 2)
    ...
    age_binary_long = np.repeat(age_binary, 2)
